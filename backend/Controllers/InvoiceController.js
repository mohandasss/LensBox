...
const generateInvoicePDFBuffer = async (orderId) => {
  try {
    console.log("📋 Generating invoice buffer for order ID:", orderId);
    ...
    return new Promise((resolve, reject) => {
      ...
      // Contact details (right aligned)
      doc
        .text('+91-6295631554', 400, companyY)
        .text('contact@lensbox.in', 400, companyY + 15)
        .text('lensbox.in', 400, companyY + 30);
      ...
      doc
        ...
        .text('Terms Net 30', 350, billToY + 35)
        .text(`Invoice date ${new Date(order.createdAt).toLocaleDateString('en-US')}`, 350, billToY + 50)
        .text(`Due date ${new Date(order.endDate || order.customerDetails?.endDate || order.createdAt).toLocaleDateString('en-US')}`, 350, billToY + 65);
      ...
      doc
        ...
        .fillColor('white')
        .fontSize(9)
        .text('Paid', 95, paymentY + 47, { width: 30, align: 'center' })
        .fillColor('black');
      ...
      if (order.razorpay?.signature) {
        doc
          ...
          .fontSize(8)
          .text('Digital Signature:', signatureX, footerY + 15, { align: 'right', width: 150 })
          .text(order.razorpay.signature.substring(0, 50) + '...', signatureX, footerY + 30, { align: 'right', width: 150 });
      }
